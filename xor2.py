...
def backward_propagate(y,y_prime,input,m,weights=None):
    ...
    if weights is not None:
        dcostdw = np.dot((np.dot(weights.T,(cost_derivative_y_prime(y,y_prime,m)*w[1]))*input*(1-input)),x.T)
        #dcostdw = np.dot(weights.T,(cost_derivative_y_prime(y,y_prime,m)*activation_derivative(y_prime)))*input*(1-input)
        dcostdb = np.sum(np.dot(weights.T,(cost_derivative_y_prime(y,y_prime,m)*w[1]))*input*(1-input),axis=1)
        return dcostdw,dcostdb.reshape(1,n_hidden).T
    else:
        dcostdw =  np.dot(cost_derivative_y_prime(y,y_prime,m)*w[1],input.T)
        dcostdb =  np.sum((cost_derivative_y_prime(y,y_prime,m)*w[1]),axis=1)
